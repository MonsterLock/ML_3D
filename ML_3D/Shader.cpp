...
void Shader::Initialize( ID3D11Device * device, HWND hwnd )
{
	...
	InitializeShader( device, hwnd, const_cast< WCHAR* >( L"C:..\\shader.vs" ), const_cast< WCHAR* >( L"C:..\\shader.fs" ) );
}
...
void Shader::Render( ID3D11DeviceContext *deviceContext, int indexCount, XMMATRIX worldMatrix, XMMATRIX viewMatrix, XMMATRIX projectionMatrix,
					 ID3D11ShaderResourceView* texture, XMFLOAT3 fLightDirection, XMFLOAT4 ambientColor, XMFLOAT4 fDiffuseColor,
					 XMFLOAT3 cameraPosition, XMFLOAT4 specularColor, float specularPower )
{
	// Set the shader parameters that it will use for rendering.
	SetShaderParameters( deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, fLightDirection, ambientColor, fDiffuseColor, cameraPosition, specularColor, specularPower );
	// Now render the prepared buffers with the shader.
	RenderShader( deviceContext, indexCount );
}

void Shader::InitializeShader( ID3D11Device * device, HWND hwnd, WCHAR * vsFilename, WCHAR * fsFilename )
{
	ID3D10Blob
	...
	TESTRESULT( FAILED( D3DCompileFromFile( vsFilename, nullptr, nullptr, "CompositeVertexShader", "vs_5_0", flags, 0, &vertexShaderBuffer, &errorMessage ) ) );

	// Compile the fragment shader code.
	TESTRESULT( FAILED( D3DCompileFromFile( fsFilename, nullptr, nullptr, "CompositeFragmentShader", "ps_5_0", flags, 0, &fragmentShaderBuffer, &errorMessage ) ) );
	...
	// Release the vertex shader buffer and fragment shader buffer since they are no longer needed.
...
void Shader::SetShaderParameters( ID3D11DeviceContext* deviceContext, XMMATRIX worldMatrix, XMMATRIX viewMatrix,
...
void Shader::RenderShader( ID3D11DeviceContext * deviceContext, int indexCount )
{
	...
	// Set the vertex and fragment shaders that will be used to render this triangle.
