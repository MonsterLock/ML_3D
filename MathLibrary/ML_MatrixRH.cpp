...
vector matrix::MatrixVectorMultiplication( const matrix & lhs , const vector & rhs )
{
	return vector(
		rhs * vector( lhs.m[m00], lhs.m[m01], lhs.m[m02], lhs.m[m03] ),
		rhs * vector( lhs.m[m10], lhs.m[m11], lhs.m[m12], lhs.m[m13] ),
		rhs * vector( lhs.m[m20], lhs.m[m21], lhs.m[m22], lhs.m[m23] ),
		rhs * vector( lhs.m[m30], lhs.m[m31], lhs.m[m32], lhs.m[m33] ) );
}

vector matrix::VectorMatrixMultiplication( const vector & lhs , const matrix & rhs )
{
	return vector(
		lhs * vector( rhs.m[m00], rhs.m[m10], rhs.m[m20], rhs.m[m30] ),
		lhs * vector( rhs.m[m01], rhs.m[m11], rhs.m[m21], rhs.m[m31] ),
		lhs * vector( rhs.m[m02], rhs.m[m12], rhs.m[m22], rhs.m[m32] ),
		lhs * vector( rhs.m[m03], rhs.m[m13], rhs.m[m23], rhs.m[m33] ) );
}
